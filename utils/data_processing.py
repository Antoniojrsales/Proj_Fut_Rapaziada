...
def process_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'Data' in df.columns:
        ...
        ).fillna(pd.to_datetime(df['Data'], format='%d/%m', errors='coerce'))

        df = df.dropna(subset=['Data_Parsed']).copy()

        ano_atual = datetime.now().year
        df['Data'] = df['Data_Parsed'].apply(
            lambda d: d.replace(year=ano_atual) if d.year == 1900 else d
        )
    ...
    if 'Odd %' in df.columns:
        df['Odd %'] = df['Odd %'].astype(str).str.replace(
            ',', '.', regex=False
        )
    ...
    df['X_Ks_num'] = pd.to_numeric(
        df['X_Ks'].astype(str).str.strip(), errors='coerce'
    )
    df['X_Fora_num'] = pd.to_numeric(
        df['X_Fora'].astype(str).str.strip(), errors='coerce'
    )
    ...
    df['X_Ks'] = np.where(
        tem_placar, df['X_Ks_num'].astype('Int64').astype(str), ''
    )
    df['X_Fora'] = np.where(
        tem_placar, df['X_Fora_num'].astype('Int64').astype(str), ''
    )

    # ⚽ Total de Gols (Apenas para partidas com placar)
    df['Gols'] = np.where(tem_placar, df['X_Ks_num'] + df['X_Fora_num'], np.nan)

    # 1. Garante tipagem numérica adequada
    df["1_gol"] = pd.to_numeric(df["1_gol"], errors="coerce")

    # 🎯 Regras de Negócio: Status Padrão (Over / Under)
    cond_over = (df['Mercado'] == 'Over_0.5FT') & (df['Gols'] > 0)
    cond_over15 = (df['Mercado'] == 'Over_1.5FT') & (df['Gols'] > 1)
    cond_under = (df['Mercado'] == 'Under_6.5FT') & (df['Gols'] <= 6)
    cond_over_ht = ((df["Mercado"] == "Over_0.5HT") 
                    & (df["1_gol"].notna())
                    & (df["1_gol"] > 0)
                    & (df["1_gol"] <= 45))    
    ...
    df['Resultado_Status'] = np.where(
        ~tem_placar,
        'Pendente',
        np.where(cond_over | cond_under | cond_over15 | cond_over_ht, 'Green', 'Red'),
    )
    ...
    # =========================================================
    # 3. APLICAÇÃO DAS REGRAS ESPECÍFICAS DE LAY
    # =========================================================
    # Chamado AQUI para sobrescrever APENAS as linhas do mercado Lay com as regras certas
    df = processar_regras_lay(df)

    # 📈 Lucro Acumulado (Soma perfeita em sequência)
    df['Lucro_R$'] = pd.to_numeric(df['Lucro_R$'], errors='coerce').fillna(0.0)
